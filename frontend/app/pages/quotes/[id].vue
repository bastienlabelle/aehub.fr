<template>
  <div class="max-w-4xl">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <div>
        <h2 class="text-xl font-bold text-base-content">{{ quote?.number }}</h2>
        <p v-if="quote?.subject" class="text-sm text-base-content/50">{{ quote.subject }}</p>
      </div>
      <span v-if="quote" class="badge ml-2" :class="statusClass(quote.status)">{{ statusLabel(quote.status) }}</span>
    </div>
...
function statusClass(status: string) {
  const classes: Record<string, string> = {
    draft: 'badge-ghost', sent: 'badge-info', accepted: 'badge-success', refused: 'badge-error', expired: 'badge-warning',
  }
...
onMounted(async () => {
  try {
    const [fetchedQuote, fetchedClients] = await Promise.all([
      $fetch<any>(`/api/quotes/${id}`, { headers: { Authorization: `Bearer ${token.value}` } }),
    ...
    Object.assign(form, {
      ...
      notes: fetchedQuote.notes ?? '',
      lines: fetchedQuote.lines.map((l: any) => ({
